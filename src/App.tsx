...
const App: React.FC = () => {
    ...
    // ▼▼▼【修正点】清書中、ファイル処理中、ビジュアライザー表示中を管理するstateを追加 ▼▼▼
    ...
    // ▼▼▼【修正点】プレビュー用ストリームとファイル処理中断フラグのrefを追加 ▼▼▼
    const previewStreamRef = useRef<MediaStream | null>(null);
    const cancelFileProcessingRef = useRef<boolean>(false);
    // ▲▲▲【修正点】▲▲▲
    ...
    const setupVisualizer = useCallback((stream: MediaStream) => {
        ...
        try {
            ...
            const draw = () => {
                if (!analyserRef.current || !canvasCtx || !audioContextRef.current || audioContextRef.current.state === 'closed') {
                    return;
                }
            ...
            console.error("ビジュアライザーのセットアップに失敗:", e);
        }
    }, [isDarkMode]);

    // ▼▼▼【修正点】マイクプレビューを停止する関数を追加 ▼▼▼
    ...
    // ▼▼▼【修正点】マイクプレビューを開始する関数を追加 ▼▼▼
    const startPreview = useCallback(async () => {
        if (isRecording || !selectedMicId) return;
        stopPreview();
        try {
            const constraints = { audio: { deviceId: { exact: selectedMicId } } };
            const stream = await navigator.mediaDevices.getUserMedia(constraints);
            previewStreamRef.current = stream;
            setupVisualizer(stream);
        } catch (err) {
            console.error("プレビュー用のマイクアクセスに失敗:", err);
            setActiveMicrophone('マイクへのアクセスが拒否されました。');
        }
    }, [isRecording, selectedMicId, setupVisualizer, stopPreview]);
    ...
    // ▼▼▼【修正点】マイク選択時にプレビューを開始するuseEffectを追加 ▼▼▼
    useEffect(() => {
        if (!isRecording) {
            startPreview();
        }
    }, [selectedMicId, isRecording, startPreview]);
    // ▲▲▲【修正点】▲▲▲
    ...
    const toggleRecording = async () => {
        if (isRecording) {
            ...
            try {
                ...
                mediaRecorderRef.current.start(10000); // 10秒ごとに文字起こし
    ...
    // ▼▼▼【修正点】ファイル処理を停止する関数を追加 ▼▼▼
    const handleStopFileProcessing = () => {
        cancelFileProcessingRef.current = true;
    };
    // ▲▲▲【修正点】▲▲▲
    ...
    const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setActiveTab(0);
        setIsTranscribing(true);
        // ▼▼▼【修正点】ファイル処理中フラグと中断フラグをセット ▼▼▼
        setIsProcessingFile(true);
        cancelFileProcessingRef.current = false;
        // ▲▲▲【修正点】▲▲▲
        setIsFileTranscriptComplete(false);
        ...
        try {
            ...
            const chunkSizeInSeconds = 60; // 60秒ごとに分割
            let currentTime = 0;
            let chunkCount = 1;
            const totalChunks = Math.ceil(duration / chunkSizeInSeconds);

            // ▼▼▼【修正点】中断フラグをループ条件に追加 ▼▼▼
            while (currentTime < duration && !cancelFileProcessingRef.current) {
            // ▲▲▲【修正点】▲▲▲
                ...
                // ▼▼▼【修正点】中断フラグをチェック ▼▼▼
                if(currentTime < duration && !cancelFileProcessingRef.current) {
                // ▲▲▲【修正点】▲▲▲
                    // APIのレート制限を考慮して待機
            ...
            // ▼▼▼【修正点】中断された場合のメッセージを追加 ▼▼▼
            if (cancelFileProcessingRef.current) {
                setModalInfo({ show: true, message: 'ファイル処理を中断しました。中断した時点までの文字起こしが完了しています。'});
            } else {
    ...
    const customStyles = `
        ...
        .ai-summary-panel { background-color: ${isDarkMode ? '#1e1e1e' : '#f0f8ff'}; color: ${isDarkMode ? '#d4d4d4' : '#333'}; padding: 25px; border-radius: 8px; border: 1px solid ${isDarkMode ? '#333' : '#cce5ff'}; min-height: 250px; white-space: pre-wrap; line-height: 1.7; }
        .ai-summary-panel h2, .ai-summary-panel h3 { color: ${isDarkMode ? '#ffffff' : '#004085'}; }
        ...
        <>
            ...
            <div className="main-container">
                ...
                <p style={{marginTop: 0}}>その場で録音するか、既存の音声ファイルを読み込んでください。</p>
                 {/* ▼▼▼【修正点】ファイル処理中は停止ボタンに切り替える ▼▼▼ */}
                <div style={{display: 'flex', gap: '10px', alignItems: 'center'}}>
                    {isProcessingFile ? (
                        <>
                            <div style={{flexGrow: 1, padding: '10px 20px', backgroundColor: '#6c757d', color: 'white', borderRadius: '5px', textAlign: 'center', fontSize: '16px' }}>
                                {loadingMessage}
                            </div>
                            <button onClick={handleStopFileProcessing} style={{ fontSize: '16px', padding: '10px 20px', backgroundColor: '#dc3545', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer' }}>
                                停止
                            </button>
                        </>
                    ) : (
                        <>
                           {/* ▼▼▼【修正点】清書中はボタンの色を緑にし、テキストを変更 ▼▼▼ */}
                            <button 
                                onClick={toggleRecording} 
                                disabled={isRefining || loadingMessage !== 'AI準備完了'} 
                                style={{ 
                                    fontSize: '16px', 
                                    padding: '10px 20px', 
                                    backgroundColor: isRefining ? '#28a745' : (isRecording ? '#dc3545' : '#007bff'),
                                    color: 'white', 
                                    border: 'none', 
                                    borderRadius: '5px', 
                                    cursor: 'pointer', 
                            ...
                            >
                                {isRefining ? 'AIが清書中です...' : (isRecording ? '■ 録音停止' : (loadingMessage !== 'AI準備完了' ? loadingMessage : '● 録音開始'))}
                            </button>
                            {/* ▲▲▲【修正点】▲▲▲ */}
                            <input type="file" ref={fileInputRef} onChange={handleFileChange} accept=".mp3,.m4a,.wav" style={{ display: 'none' }} />
                            <button onClick={() => fileInputRef.current?.click()} disabled={(loadingMessage !== 'AI準備完了') || isRecording || isRefining} style={{ fontSize: '16px', padding: '10px 20px', backgroundColor: '#6c757d', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer', flex: 1 }}>
                                音声ファイルを読み込む
                            </button>
                        </>
                    )}
                ...
                {/* ▼▼▼【修正点】ビジュアライザーの表示条件を変更 ▼▼▼ */}
                <canvas ref={canvasRef} style={{ width: '100%', height: '60px', marginTop: '15px', borderRadius: '5px', display: isVisualizerActive ? 'block' : 'none' }}></canvas>
                {/* ▲▲▲【修正点】▲▲▲ */}
                    ...
                    <TabPanel>
                        ...
                        <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', marginBottom: '10px'}}>
                            ...
                            {!isRecording && transcript.length > 0 && !isTranscribing && (
                                <button onClick={handleRefineTranscript} disabled={isRefining} style={{fontSize: '14px', padding: '8px 16px', backgroundColor: '#28a745', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer' }}>
                                    {isRefining ? '清書中...' : 'AIで清書する'}
                                </button>
                            )}
